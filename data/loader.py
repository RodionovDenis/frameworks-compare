...
class DryBean(TextParser):
    def __init__(self):
        super().__init__('dry-bean/Dry_Bean_Dataset.csv', spliter=',')

    def load_dataset(self) -> Dataset:
        features, targets = self.parse_file(sample_skip=0,  index_target=-1)
        return Dataset('Dry Bean', features, targets)
    

class MagicGammaTelescope(TextParser):
    def __init__(self):
...
class CylinderBands(TextParser):
    ...
    def load_dataset(self) -> Dataset:
        features, targets = self.parse_file(index_target=-1, feature_skip=[0, 1])
        return Dataset('Cylinder Bands', features, targets)


class ConnectionBenchVowel(TextParser):
    def __init__(self):
...
class Banana(ArffParser):
    def __init__(self):
        super().__init__('banana.arff')
